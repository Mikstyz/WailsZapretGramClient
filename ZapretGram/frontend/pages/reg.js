...
(() => {
    ...
    function updateButtonState() {
        const valid = nameEl.value.trim().length >= 6 && pwEl.value.trim().length >= 8;
    ...
    function renderStrength() {
        ...
        if (score > 0)
            strengthBarEl.classList.add("s" + score);
        if (!pw) {
    ...
    togglePwEl.addEventListener("click", () => {
        ...
        togglePwEl.setAttribute("aria-label", isPwd ? "Скрыть пароль" : "Показать пароль");
    });
