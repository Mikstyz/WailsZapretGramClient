...
(() => {
    ...
    const truncate = (text, max = 64) => text.length > max ? `${text.slice(0, max - 1).trimEnd()}…` : text;
    ...
    const matchesChat = (chat, normalizedQuery) => {
        ...
        return chat.messages.some((message) => message.text.toLowerCase().includes(normalizedQuery));
    };
    const matchesUser = (user, normalizedQuery) => normalizedQuery ? user.name.toLowerCase().includes(normalizedQuery) : false;
    ...
    const updateSendButtonState = () => {
        const shouldDisable = composerInput.disabled || composerInput.value.trim().length === 0;
        sendBtn.disabled = shouldDisable;
    };
    const renderChatList = () => {
        const query = state.searchTerm.trim().toLowerCase();
        const chatMatches = state.chats.filter((chat) => matchesChat(chat, query));
        const sortedChats = [...chatMatches].sort((a, b) => getLastActivityTimestamp(b) - getLastActivityTimestamp(a));
        const existingChatIds = new Set(state.chats.map((chat) => chat.id));
        const suggestionUsers = query
            ? state.users.filter((user) => !existingChatIds.has(user.id) && matchesUser(user, query))
            : [];
        ...
        if (sortedChats.length > 0) {
            ...
            sortedChats.forEach((chat) => {
                ...
                indicator.className = `indicator ${chat.isOnline ? "online" : "offline"}`;
        ...
        if (suggestionUsers.length > 0) {
            ...
            suggestionUsers.forEach((user) => {
                ...
                nameEl.className = "name";
                const indicator = document.createElement("span");
                indicator.className = `indicator ${user.isOnline ? "online" : "offline"}`;
    ...
    const renderConversation = (chat) => {
        ...
        const orderedMessages = [...chat.messages].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
        const q = state.chatMessageSearch.trim().toLowerCase();
        const filtered = q
            ? orderedMessages.filter((m) => m.text.toLowerCase().includes(q))
            : orderedMessages;
        if (filtered.length === 0) {
            fragment.append(createEmptyState(q
    ...
    const openUserModal = (user) => {
        ...
        const setActiveTab = (id) => {
            ...
            modal
                .querySelectorAll(".user-tab-panel")
                .forEach((panel) => {
